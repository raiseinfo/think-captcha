...
class Captcha
{
    ...
    protected function generate(): array
    {
        ...
        $this->cache->store(config('cache.default'))->set($ckey,$key,$this->expire);
    ...
    {
        if (!$this->cache->store(config('cache.default'))->has($ckey)) {
            return false;
        }

        $key = $this->cache->store(config('cache.default'))->get($ckey);

        $code = mb_strtolower($code, 'UTF-8');

        if ($code == $key) {
            $this->cache->store(config('cache.default'))->delete($ckey);
			return true;
        }
